...
const seedDatabase = async () => {
  try {
    ...
    // Insert categories first to get real ObjectIds
    ...
    // Build numeric id → ObjectId map
    const categoryIdMap = {};
    categories.forEach((c, i) => { categoryIdMap[c.id] = categoryDocs[i]._id; });
    ...
    const transformedProducts = products.map(product => ({
      ...
      sizes: Array.isArray(product.sizes)
        ? product.sizes.map(s => typeof s === 'string' ? { name: s, price: 0 } : s)
        : [],

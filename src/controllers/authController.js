  ...
  try {
    ...
    if (!emailResult.success) {
      if (process.env.NODE_ENV === 'development') {
        console.warn(`OTP email delivery failed in development. Fallback OTP for ${email}: ${otp}`);
        return res.status(200).json({
          success: true,
          message: 'Verification code generated (email delivery unavailable in development mode).',
          devOtp: otp
        });
      }
...
const resendOTP = async (req, res) => {
  try {
    ...
    if (!emailResult.success) {
      if (process.env.NODE_ENV === 'development') {
        console.warn(`Resend OTP email delivery failed in development. Fallback OTP for ${email}: ${otp}`);
        return res.status(200).json({
          success: true,
          message: 'Verification code regenerated (email delivery unavailable in development mode).',
          devOtp: otp
        });
      }

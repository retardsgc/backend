...
const requestReturn = async (req, res) => {
  try {
    ...
    const deliveredDate = order.shippingInfo?.actualDelivery || order.updatedAt;
    ...
    order.timeline.push({
      ...
      details: `Return requested: ${message.trim()}`,
      performedAt: new Date(),
...
const createGuestOrder = async (req, res) => {
  try {
    const { items, shippingAddress, customerEmail, customerName, customerPhone, orderNotes, paymentMethod } = req.body;
    ...
    const order = await Order.create({
      ...
      guestName: customerName || shippingAddress.fullName,
      guestPhone: customerPhone || shippingAddress.phone,
      shippingAddress: {
        ...shippingAddress,
...
// Get guest order by ID and email (no auth required)
const getGuestOrder = async (req, res) => {
  try {
    ...
    const order = await Order.findOne(query).populate('items.product', 'name images category');
    if (!order) {
      return res.status(404).json({ success: false, message: 'Order not found' });
    }
    return res.status(200).json({ success: true, data: order });
